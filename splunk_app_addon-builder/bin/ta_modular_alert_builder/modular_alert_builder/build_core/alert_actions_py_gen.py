...
class AlertActionsPyCleaner(AlertActionsPyBase):

    def __init__(self, input_setting=None, package_path=None, logger=None,
                 deleted_alerts=None, lookup_dir=None, global_settings=None,
                 **kwargs):
        super(AlertActionsPyCleaner, self).__init__(
            input_setting=input_setting, package_path=package_path,
            logger=logger, lookup_dir=lookup_dir,
            global_settings=global_settings, **kwargs)
        self._deleted_alerts = deleted_alerts

    def clean_alert(self):
        delete_files = []
        delete_files.append(self.get_alert_py_path())
        delete_files.append(self.get_alert_helper_py_path())
        for delete_file in delete_files:
            if op.exists(delete_file) and op.isfile(delete_file):
                remove(delete_file)
                self._logger.info('event="Clean alert py file: %s"',
                                  delete_file)
            if op.exists(delete_file + "c") and op.isfile(delete_file + "c"):
                remove(delete_file + "c")
            if op.exists(delete_file + "o") and op.isfile(delete_file + "o"):
                remove(delete_file + "o")
    ...
    def clean(self):
        ...
        if not self._alert_actions_setting or len(self._alert_actions_setting) <=0:
            clean_files = self.get_template_py_files()
            self._logger.info('clean_files %s"', clean_files)
            for cfile in clean_files:
                if op.exists(cfile):
                    remove(cfile)
                    self._logger.info('event="Cleaned file %s"', cfile)
...
class AlertActionsPyGenerator(AlertActionsPyBase):
    ...
    def gen_declare_py_file(self):
        ...
        rendered_content = template.render(app_name=self._ta_name,
                                           python_lib_dir=self._lib_dir)

        self._logger.debug('operation="Writing file", file="%s"',
                           self.get_decalre_py_path())
        write_file(self.get_declare_py_name(),
                   self.get_decalre_py_path(),
                   rendered_content,
                   self._logger)
...
def clean_alert_actions_py_files(input_setting=None, package_path=None,
                                 logger=None, global_settings=None,
                                 **kwargs):
    py_cleaner = AlertActionsPyCleaner(input_setting=input_setting,
                                       package_path=package_path,
                                       logger=logger,
                                       global_settings=global_settings,
                                       **kwargs)
