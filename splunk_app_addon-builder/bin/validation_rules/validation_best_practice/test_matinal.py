...
def _create_result(rid, params=None):
    if params:
        escaped_params = {k: _escape_str(v) for k, v in list(params.items())}
...
class TestMatinal(object):
        ...
        if self.utils.file_exist('default/commands.conf'):
            ...
            if len(stanza_list) != len(set(stanza_list)):
                r1 = _create_result(
                    8012, {'stanza_name':
                           str([item
                                for item, count in list(collections.Counter(
    ...
    def test_file_in_top_directory(self, results=None):
        ...
        check_file_list = ['README.txt', 'license-eula.rtf',
                           'license-eula.txt', 'app.manifest',
                           f'{self.appname}.aob_meta', 'aob_events_in_meta.json']
        ...
        if len(top_files) != 0:
            r1 = _create_result(8015, {'unneeded_files': ",".join(top_files)})
            self.logger.info(r1)
            results.append(r1)
    ...
    # Disable this rule since TAB-1263: conf parser cannot read duplicate stanzas
    ...
    def test_no_global_default_definition_in_conf(self, results=None):
        ...
        app_namespace = re.sub('[^\w]', '_', self.appname)
        skip_file_list = [app_namespace + '_settings.conf', app_namespace + '_account.conf']

        all_conf = self.utils.get_files('default', '.*\.conf')
        for f in all_conf:
            if f in skip_file_list:
                continue
